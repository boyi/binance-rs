...
impl FuturesMarket {
    ...
    pub async fn get_depth<S>(&self, symbol: S) -> Result<OrderBook, BinanceError>
    where
        S: Into<String>,
    {
    ...
    pub async fn get_custom_depth<S>(&self, symbol: S, depth: u64) -> Result<OrderBook, BinanceError>
    where
        S: Into<String>,
    {
        let mut parameters: BTreeMap<String, String> = BTreeMap::new();
    ...
    pub async fn get_trades<S>(&self, symbol: S) -> Result<Trades, BinanceError>
    where
        S: Into<String>,
    {
        let mut parameters: BTreeMap<String, String> = BTreeMap::new();
    ...
    pub async fn get_24h_price_stats<S>(&self, symbol: S) -> Result<PriceStats, BinanceError>
    where
        S: Into<String>,
    {
        let mut parameters: BTreeMap<String, String> = BTreeMap::new();
    ...
    pub async fn get_all_24h_price_stats(&self) -> Result<Vec<PriceStats>, BinanceError> {
        self.client.get(API::Futures(Futures::Ticker24hr), None).await
    }

    // Latest price for ONE symbol.
    pub async fn get_price<S>(&self, symbol: S) -> Result<SymbolPrice, BinanceError>
    where
        S: Into<String>,
    {
        let mut parameters: BTreeMap<String, String> = BTreeMap::new();
    ...
    pub async fn get_all_prices(&self) -> Result<crate::model::Prices, BinanceError> {
        self.client.get(API::Futures(Futures::TickerPrice), None).await
    }

    // Symbols order book ticker
    // -> Best price/qty on the order book for ALL symbols.
    pub async fn get_all_book_tickers(&self) -> Result<BookTickers, BinanceError> {
        self.client.get(API::Futures(Futures::BookTicker), None).await
    }

    // -> Best price/qty on the order book for ONE symbol
    pub async fn get_book_ticker<S>(&self, symbol: S) -> Result<Tickers, BinanceError>
    where
        S: Into<String>,
    {
        let mut parameters: BTreeMap<String, String> = BTreeMap::new();
    ...
    pub async fn get_mark_prices(&self) -> Result<MarkPrices, BinanceError> {
        self.client.get(API::Futures(Futures::PremiumIndex), None).await
    }

    pub async fn get_all_liquidation_orders(&self) -> Result<LiquidationOrders, BinanceError> {
        self.client.get(API::Futures(Futures::AllForceOrders), None).await
    }

    pub async fn open_interest<S>(&self, symbol: S) -> Result<OpenInterest, BinanceError>
    where
        S: Into<String>,
    {
        let mut parameters: BTreeMap<String, String> = BTreeMap::new();
